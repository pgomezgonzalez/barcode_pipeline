...
def cli():
	...
	if args.nBarcodes == "single":
		...
		sp.run(r'''cat list_bams | parallel -j 1 "if [[ -f ./mapping/{}.mapped.bam ]]; then samtools index ./mapping/{}.mapped.bam; fi"''',shell=True,executable='/bin/bash')
	...
	#################################################################### IF DUAL BARCODES ARE USED -> --nBarcodes dual ####################################################################	
		...
		for barcode_dir in demux_root.iterdir():
			...
			sp.run(rf'''cat list_bams_{barcode} | parallel -j 1 "if [[ -f {out_dir}/{{}}.mapped.bam ]]; then samtools index {out_dir}/{{}}.mapped.bam; fi"''',shell=True,executable='/bin/bash')
			print("...counting reads...")
	...
	sp.run(rf'for f in internal_barcodes*; do [ -e "$f" ] && mv "$f" "{args.prefix}.$f"; done', shell=True, check=False)
	...
	sp.run(rf'for f in mean_coverage*.bed; do [ -e "$f" ] && mv "$f" "{args.prefix}.$f"; done',shell=True, check=False)
	#sp.run(f'mv rest_coverage.bed {args.prefix}.rest_coverage.bed', shell=True)
